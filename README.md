...
| `case_authorities` | `opinion_id` (required), `limit`, `cursor` | The authorities an opinion relies on (its table of authorities) with a per-authority citation depth, via `/opinions-cited/`. Token required. Also returns `next_cursor` — pass it back as `cursor` for the next page; `/opinions-cited/` defers its `count`, so the cursor is the only signal that the table is longer than one page. |
...
`dropped_from_this_page` and the `note` are why `limit: 1` is a bad way to page. `/search/` serves one fixed ~20-row page and ignores `page_size`, so `limit` slices client-side while `next_cursor` starts after the whole page — paging from here would skip the 19 rows the slice dropped. Raise `limit` to 20 when you intend to page.
...
Counts as of 2026-09-14 (fix round 1): 200 tests in 4 files (`npm test`), 2713 lines of app source and packaging scripts, 2479 lines of test source.
