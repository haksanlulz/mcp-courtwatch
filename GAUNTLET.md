...
6. **The upstream-contract rung cannot go fully green on this account** (2026-09-14). `npm run smoke` now makes one real call per tool — twelve requests, eleven checks — and CourtListener's hourly ceiling for a new token is 50; a run inside a spent hour reports FAIL on whatever the ceiling refuses, which is correct behavior and still means the eleven checks have not been observed green *together*. Each has been observed green; that is a weaker claim and is the one this file makes. It resolves itself as the account ages, or on an older token.
